...
pub fn build_pipeline_base(
    ...
    const vert_shader_code = try read_file(vertex_shader_filename, allocator);
    defer allocator.free(vert_shader_code);
    const frag_shader_code = try read_file(fragment_shader_filename, allocator);
    ...
    const renderpass = system.renderpass_system.get_renderpass_from_name(renderpass_name) orelse unreachable;
